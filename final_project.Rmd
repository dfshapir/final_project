...
# I decided to make the year equal 2010 to give the most recent overview of data.
...
# There was a lot of excess data in here. I didn't need it. I also filtered out "Maga Buryatdan" and "Nenets" because both of them were elided into different provinces in the actual data. Below, I had to recode all of the names so that I could match the two sets together. I thought long and hard about how to do this more efficiently, but this does the trick for this data set. Obviously, for longer data sets, it just would have been infeasible. But for one with such a small number of values, I figured it would work. I sincerely apologize for all of the code.
...
shapedat <- ggplot(data = Shape, aes(geometry = geometry, fill = BIRTH)) +
  geom_sf()
